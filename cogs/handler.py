...
class handler(commands.Cog):
    ...
    async def fetch_close_by(self, message: discord.Message):
        if (
            message.author.id != 864969115839758356
            or message.channel.id != 851314198654484521
        ):
            return
        if not (
            match := re.search(
                r"FETCH_NEARBY \| (?P<X>-?\d+) \| (?P<z>-?\d+) \| (?P<Name>\w+) \| (?P<radius>-?\d+)",
                message.content,
            )
        ):
            return
        ...
        if not results:
            return await message.channel.send(
                """!xc tellraw insert_player_here ["",{"text":"[","bold":true,"color":"blue"},{"text":"discord","color":"aqua"},{"text":"] ","bold":true,"color":"blue"},{"text":"No results founds within a insert_radius_here block radius!","color":"red"}]
            """.replace(
                    "insert_radius_here", radius
                ).replace(
                    "insert_player_here", name
                )
            )
        lines = [
            """{"text":"\\nxcoord","color":"yellow"},{"text":"X","color":"gold"},{"text":" zcoord","color":"yellow"},{"text":"Z","color":"gold"},{"text":" - description","color":"gray"}""".replace(
                "xcoord", str(x)
            )
            .replace("zcoord", str(z))
        ...
        page = (
            """!xc tellraw insert_player_here ["",{"text":"header_here","color":"blue"}table_thing{"text":"\\n---------------------------------amountstr","color":"blue"}]
        """.replace(
                "header_here", header
            )
            .replace("insert_player_here", name)
            .replace("table_thing", page)
            .replace("amountstr", str("-" * len(str(radius))))
        )
        if len(page) <= 2000:
            await message.channel.send(page)
        else:
            return await message.channel.send(
                """!xc tellraw insert_player_here ["",{"text":"[","bold":true,"color":"blue"},{"text":"discord","color":"aqua"},{"text":"] ","bold":true,"color":"blue"},{"text":"The amount of characters exceeded the amount of characters allowed! Please contact Leo and tell him to fix it.","color":"red"}]
            """.replace(
                    "insert_radius_here", radius
                ).replace(
                    "insert_player_here", name
    ...
    async def insert_into_database(self, message: discord.Message):
        ...
        if not (
            match := re.search(
                r"^(?P<X>-?[0-9]*) \| (?P<Z>-?[0-9]*) \| (?P<Name>\w+) \| (?P<UUID>[0-9a-f]{8}[-]?[0-9a-f]{4}[-]?[0-9a-f]{4}[-]?[0-9a-f]{4}[-]?[0-9a-f]{12}) \| (?P<Description>.*)$",
                message.content,
            )
        ):
            return
        x, z, name, uuid, description = (
            match.group("X"),
            match.group("Z"),
            match.group("Name"),
            match.group("UUID"),
        ...
        author_id = await self.bot.db.fetchval(
            "SELECT user_id FROM usernames WHERE minecraft_id = $1", uuid
        )
        if not author_id:
            await message.channel.send(
                """!xc tellraw insert_minecraft_username ["",{"text":"[","bold":true,"color":"blue"},{"text":"discord","color":"aqua"},{"text":"]","bold":true,"color":"blue"},{"text":" Succesfully saved to discord database as ","color":"gold"},{"text":"insert_discord_username ","color":"yellow"},{"text":"with annotation ","color":"gold"},{"text":"insert_note_here","color":"yellow"}]
            """.replace(
                    "insert_minecraft_username", name
                )
            )
            return
        try:
            ...
            """.replace(
                ...
                .replace(
                    "insert_discord_username",
                    str(
                        self.bot.get_user(author_id)
                        or f"User not found (ID: {author_id})"
                    ),
                )
                .replace("insert_note_here", description)
            )
        except asyncpg.UniqueViolationError:
            coords_author_id = await self.bot.db.fetchval(
                "SELECT author FROM coords WHERE x = $1 AND z = $2", int(x), int(z)
            )
            if coords_author_id:
                await message.channel.send(
                    """!xc tellraw insert_minecraft_username ["",{"text":"[","bold":true,"color":"blue"},{"text":"discord","color":"aqua"},{"text":"]","bold":true,"color":"blue"},{"text":" Sorry but ","color":"red"},{"text":"insert_discord_username ","color":"yellow"},{"text":"has already saved these coordinates. ","color":"red"},{"text":"Maybe move a bit?","color":"yellow"}]
                """.replace(
                        "insert_discord_username",
                        str(
                            self.bot.get_user(coords_author_id)
                            or f"User not found (ID: {coords_author_id})"
                        ),
                    ).replace(
                        "insert_minecraft_username", name
                    )
                ...
                await message.channel.send(
                    """!xc tellraw insert_minecraft_username ["",{"text":"[","bold":true,"color":"blue"},{"text":"discord","color":"aqua"},{"text":"]","bold":true,"color":"blue"},{"text":" Sorry but ","color":"red"},{"text":"insert_discord_username ","color":"yellow"},{"text":"has already saved these coordinates. ","color":"red"},{"text":"Maybe move a bit?","color":"yellow"}]
                """.replace(
                        "insert_discord_username", "UNKNOWN USER"
                    ).replace(
                        "insert_minecraft_username", name
                    )
                )
