...
class moderation(commands.Cog):
    ...
    async def autounmute(self):
        ...
        next_task = await self.bot.db.fetchrow(
            "SELECT * FROM selfmutes ORDER BY end_time LIMIT 1"
        )
        ...
        await umember.remove_roles(
            urole, reason=f"end of self-mute for {umember} ({umember.id})"
        )

        await self.bot.db.execute(
            "DELETE FROM selfmutes WHERE member_id = $1", next_task["member_id"]
        )
    ...
    async def mute(
        ...
        try:
            ...
            mem_embed.set_author(
                name=f"You've been muted by a staff member",
...
```reason: {reason}```""",
    ...
    async def unmute(
        ...
        try:
            ...
            mem_embed = discord.Embed(color=ctx.me.color)
            mem_embed.set_author(
                name=f"You've been unmuted by a staff member",
...
```reason: {reason}```""",
    ...
    async def selfmute(self, ctx, *, duration: helpers.ShortTime):
        ...
        try:
            await ctx.author.add_roles(
                urole,
                reason=f"{ctx.author} ({ctx.author.id}) muted themselves for {delta}",
            )
        except:
            return await ctx.send("something went wrong...")

        test_str = await self.bot.db.fetchrow(
            "SELECT * FROM selfmutes WHERE member_id = $1", ctx.author.id
        )
        if not test_str:
            await self.bot.db.execute(
                "INSERT INTO selfmutes(member_id, end_time) VALUES ($1, $2)",
                ctx.author.id,
                duration.dt,
            )
        else:
            await self.bot.db.execute(
                "UPDATE selfmutes SET end_time = $1 WHERE member_id = $2",
                duration.dt,
                ctx.author.id,
            )
    ...
    async def GameBan(
        self, ctx, argument: typing.Optional[str] = "invalid name", *, reason=None
    ):
        if not any(role in self.staff_roles for role in ctx.author.roles):
            await self.perms_error(ctx)
            return
        ...
        if name:
            if reason:
                ...
                embed = discord.Embed(
                    description=f"""{ctx.author.mention} banned **{name}** from the server
```reason: {reason}```""",
                    color=ctx.me.color,
                )
            else:
    ...
    async def GameUnban(
        ...
        if name:
            if reason:
                ...
                embed = discord.Embed(
                    description=f"""{ctx.author.mention} unbanned **{name}** from the server
```reason: {reason}```""",
                    color=ctx.me.color,
                )
            else:
    ...
    async def vcban(
        ...
        try:
            ...
            mem_embed = discord.Embed(color=ctx.me.color)
            mem_embed.set_author(
                name=f"You've been VC-Banned by a staff member",
                icon_url="https://i.imgur.com/hKNGsMb.png",
            )
            mem_embed.set_image(url="https://i.imgur.com/hXbvCT4.png")
            if reason:
...
```reason: {reason}```""",
    ...
    async def vcunban(
        ...
        try:
            ...
            mem_embed = discord.Embed(color=ctx.me.color)
            mem_embed.set_author(
                name=f"You've been VC-Unbanned by a staff member",
                icon_url="https://i.imgur.com/m1MtOVS.png",
            )
            mem_embed.set_image(url="https://i.imgur.com/23XECtg.png")
            if reason:
            ...
            if reason:
                embed = discord.Embed(
                    description=f"""{ctx.author.mention} VC-Unbanned {member.mention}
```reason: {reason}```""",
                    color=ctx.me.color,
                )
            else:
