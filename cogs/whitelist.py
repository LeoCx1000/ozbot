    ...
    async def convert(_, argument):
        if argument not in list(unicode_emoji.EMOJI_UNICODE_ENGLISH.values()):
            raise commands.BadArgument(
                f"failed to convert {argument} into a valid unicode emoji."
            )
        return argument
    ...
    async def convert(cls, ctx: commands.Context, argument: str):
        async with ctx.bot.session.get(
            ...
            if r.status in (204, 400, 404):
                raise commands.BadArgument(
                    f"{argument[0:100]} is not a valid minecraft name."
                )

            elif r.status != 200:
                raise commands.BadArgument(
                    f"{argument[0:100]} is not a valid minecraft name. (Status cde {r.status})"
                )
...
class Whitelist(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        with contextlib.suppress(discord.HTTPException):
            ...
            if await self.bot.db.fetchval(
                "select user_id from usernames where user_id = $1", message.author.id
            ):
            ...
            if message.guild.get_role(833843541872214056) in message.author.roles:
                return await message.channel.send(
                    "⚠ Sorry but you can't do that! you're already whitelisted.",
                    delete_after=5,
                )
            await message.delete(delay=0.2)
            ...
            if r.status in (204, 400):
                ...
                try:
                    await self.bot.db.execute(
                        "insert into usernames (user_id, minecraft_id) values ($1, $2)",
                        message.author.id,
                        uuid,
                    )
                except asyncpg.UniqueViolationError:
                    uuid = await self.bot.db.fetchval(
                        "select minecraft_id from usernames where user_id = $1",
                        message.author.id,
                    )
                    if uuid:
                        await message.channel.send(
                            f"Your discord user id is already in the whitelist database. You will receive the roles."
                            f"\nIf you cannot log on, warn an admin.",
                            delete_after=10,
                        )
                    else:
                        await message.channel.send(
                    ...
                    await message.author.add_roles(
                        message.guild.get_role(833843541872214056),
                        message.guild.get_role(798698824738668605),
                    )
                    try:
                        await message.author.remove_roles(
                            message.guild.get_role(851593341409820722)
                        )
                    except Exception as e:
                        logging.error(
                            f"Could not remove role from {message.author}!", exc_info=e
                        )
                    return
                ...
                embed2 = discord.Embed(
                    description=f"Automatically added user `{user}` to the whitelist",
                    color=0x75AF54,
                )
                embed2.set_footer(
                ...
                embed.add_field(
                    name=f"✅ YOU HAVE ACCEPTED THE RULES AND YOU HAVE BEEN WHITELISTED",
                ...
                embed = discord.Embed(color=0x75AF54)
                embed.add_field(
                    name=f"❌ Server is offline, try again in a few minutes",
    ...
    async def whitelist_add(self, ctx, member: discord.Member, mc_user: MinecraftName):
        try:
            ...
            if uuid:
                await ctx.send(
                    f"That minecraft name is already linked to an account with a UUID of {uuid}"
                )
            else:
                dc_id = await self.bot.db.fetchval(
                    "select user_id from usernames where minecraft_id = $1",
                    mc_user.uuid,
                )
                user = self.bot.get_user(dc_id)
                await ctx.send(
                    f"That minecraft name is already saved by {user or dc_id}"
                )
            return
        await ctx.send("Linked.")
        await member.add_roles(
            ctx.guild.get_role(833843541872214056),
    ...
    async def whitelist_remove(
        ...
        if isinstance(member, discord.Member):
            q = await self.bot.db.execute(
                "DELETE FROM usernames WHERE user_id = $1", member.id
            )
            await ctx.send(f"Done. `{q}`")
        elif isinstance(member, MinecraftName):
            q = await self.bot.db.execute(
                "DELETE FROM usernames WHERE minecraft_id = $1", member.uuid
            )
            await ctx.send(f"Done. `{q}`")
        else:
            await ctx.send(
                "something went wrong... idk why. <@349373972103561218> fix it.",
    ...
    async def react(
        ...
        *emojis: typing.Union[discord.Emoji, UnicodeEmoji],
    ):
        with contextlib.suppress(discord.HTTPException):
    ...
    async def automatic_whitelist_remover(self, member: discord.Member):
        ...
        await logs.send(
            f"Removed {member} ({name} / {uuid}) from the whitelist automatically."
        )
