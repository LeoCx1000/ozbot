...
class ModMail(commands.Cog):
    """
    🤨 Functions to send and receive Bot's DMs.
    ...
    def forum(self) -> discord.ForumChannel:
        channel = self.bot.get_channel(constants.MODMAIL_FORUM)
    ...
    def welcome_embed(self) -> discord.Embed:
        embed = discord.Embed(
            ...
            title="Welcome to OZ SMP's StaffMail",
            description=(
                "Hello there! Thanks for contacting the OZ team. "
                "This conversation will be kept private between you "
                "and the team. We will reply to you at our "
                "soonest convenience. Thank you for choosing OZ! 💞"
            ),
        )
        embed.add_field(
            name="Limitations:",
            value=(
                "This will only track initial messages.\n __Message edits, deletions, "
                "reactions, and other complex chat features are not supported__ (yet). \n"
                "For now, only **message content**, and **attachments** (that are "
                "less than 8mb in size) will be forwarded to the team. Not other things "
                "like stickers, etc."
            ),
        )
        return embed
    ...
    async def process_dm(self, message: discord.Message):
        try:
            ...
            if constants.NO_CHAT_TAG in thread._applied_tags:
                return await message.author.send(
                    embed=discord.Embed(
                        description="You are blocked from DMing the team.",
                        color=discord.Color.red(),
                    )
                )
    ...
    async def process_reply(self, message: ThreadMessage):
        if constants.NO_CHAT_TAG in message.channel._applied_tags:
            await message.add_reaction("\N{WARNING SIGN}")
