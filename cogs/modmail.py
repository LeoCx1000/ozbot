...
class modmail(commands.Cog):
    ...
    async def modmail(self, message):
        ...
        category = self.bot.get_guild(706624339595886683).get_channel(
            879414245052284958
        )
            ...
            for attachment in message.attachments:
                if attachment.size > 8388600:
                    await message.author.send(
                        "Sent message without attachment! File size greater than 8 MB."
                    )
                    continue
    ...
    async def modmail_reply(self, message):
        ...
        if any(
            (
                not message.guild,
                message.author.bot,
                getattr(message.channel, "category_id", 0) != 879414245052284958,
            )
        ):
            return
        ...
        try:
            user = self.bot.get_user(int(channel.name)) or (
                await self.bot.fetch_user(int(channel.name))
            )
    ...
    async def dm(self, ctx, member: discord.Member, *, message=None):
        ...
        if ctx.channel.category_id == 879414245052284958:
            return
    ...
    def _update_staff_roles(self):
        ...
        for roleid in self.yaml_data["StaffRoles"]:
            staff_roles.append(
                self.bot.get_guild(self.yaml_data["guildID"]).get_role(roleid)
            )
        self.staff_roles = staff_roles
